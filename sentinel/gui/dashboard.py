...
class SentinelDashboard:
    ...
    - Auto-restart capability with checkbox toggle
    ...
    def _create_control_panel(self, parent):
        ...
        auto_restart_frame.pack(pady=(0, 10))
    ...
    def _start_cthulu(self):
        """Start Cthulu button handler"""
        if self.guardian:
            def start_with_feedback():
                try:
                    self.root.after(0, lambda: self.status_text.config(text="🚀 Starting Cthulu..."))
                    self.root.after(0, lambda: self.start_btn.config(state='disabled'))
                    result = self.guardian.start_cthulu()
                    if result:
                        self.root.after(0, lambda: self.status_text.config(text="✅ Cthulu started successfully"))
                    else:
                        self.root.after(0, lambda: self.status_text.config(text="⚠️ Cthulu start failed - check logs"))
                except tk.TclError:
                    pass  # Window closed
                except Exception as e:
                    try:
                    ...
                    try:
                        ...
                        pass
            threading.Thread(target=start_with_feedback, daemon=True).start()
        else:
            messagebox.showinfo("Info", "Guardian not connected - standalone mode")
    
    def _stop_cthulu(self):
        """Stop Cthulu button handler"""
        if self.guardian:
            if messagebox.askyesno("Confirm", "Stop Cthulu trading system?"):
                def stop_with_feedback():
                    try:
                        self.root.after(0, lambda: self.status_text.config(text="🛑 Stopping Cthulu..."))
    ...
    def _force_recovery(self):
        """Force recovery button handler"""
        if self.guardian:
            if messagebox.askyesno("Confirm", "Force recovery procedure?"):
                def recover_with_feedback():
                    try:
                        self.root.after(0, lambda: self.status_text.config(text="🔄 Running recovery..."))
                        self.root.after(0, lambda: self.recover_btn.config(state='disabled'))
                        result = self.guardian.recover_from_crash()
                        if result:
                            self.root.after(0, lambda: self.status_text.config(text="✅ Recovery completed"))
                        else:
