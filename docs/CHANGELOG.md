---
title: Changelog
description: Herald project release notes and version history
---
...
All notable changes to the Herald Adaptive Trading Intelligence project are documented here. This file follows the Keep a Changelog conventions and uses Semantic Versioning (https://semver.org/).
...
### Added
- Placeholder for unreleased features (use the template below)

### Changed
- Notes for breaking changes, updates, or refactors

### Fixed
- General fixes (bugs, test improvements)
...
### Summary
Enhances trade management capabilities, adds CLI tooling, improves configuration validation, and introduces production observability endpoints.

### Added
- External trade adoption (`position/trade_manager.py`) — Detect and manage externally created trades with configurable adoption policy (filters, age limits, log-only mode).
- Command-line trade management (`scripts/trade_cli.py`) — Place, list and close positions from the CLI with optional dry-run.
  ...
  ```
- Predefined risk profiles (`config/mindsets.py`) — Quick presets for aggressive/balanced/conservative trading.
...
- Observability: `observability/health.py` (health checks), `observability/prometheus.py` (Prometheus metrics).

### Changed
- Position reconciliation at startup now detects and tracks pre-existing positions across sessions and applies exit strategies consistently.
- Exit strategies: output and priority semantics normalized (consistent `ExitDecision` return types and priority ordering).

### Security
- Pre-commit hooks configured with `detect-secrets`, plus `.secrets.baseline` for scanned secrets. Sensitive values are masked in logs.

### Files added
- `position/trade_manager.py`, `scripts/trade_cli.py`, `scripts/force_trade.py`, `config/mindsets.py`, `config_schema.py`, `observability/health.py`, `observability/prometheus.py`, `.pre-commit-config.yaml`, `.secrets.baseline`, `tests/unit/test_trade_manager.py`.

### Fixed
- Normalized return types and priority semantics for exit strategies.
- Reconciled `get_pnl_pips()` defaults and updated callers.
- Documentation corrected to use JSON config examples where required.

### Testing
- Added / updated unit tests for Trade Manager and exit strategies. Existing test suite remains green.
...
### Summary
Production-ready system release: Phase 2 full completion with robust MT5 integration, complete test coverage and CI automation.

### Added
- ATR indicator and enhancements across indicators (RSI, MACD, Bollinger, Stochastic, ADX).
- Exit Manager and exit strategies for stop loss, take profit, trailing and time-based exit.
- GitHub Actions-based CI: multi-python matrix, linting and coverage reporting.

### Changed
- Integration tests made visible and refactored to use pytest fixtures and improved lifecycle management.
- Versioning and documentation updated for production deployability.

### Fixed
- Improved MT5 connection handling, market data compatibility, and test convention fixes.

### Security & Stability
- Credentials and secrets externalized to environment variables; import-time resilience for dev environments without MT5.
...
### Added
- Brief feature description, optionally with file path(s).

### Changed
- Behavior and configuration changes with migration guidance where relevant.

### Deprecated
- Deprecated features and planned removal notes.

### Removed
- Details about removed features and alternatives.
...
- Use the sections above (Added/Changed/Deprecated/Removed/Fixed/Security) and concise bullet points.
- Specify file paths and PR links where helpful.
- Prefer present-tense, one-sentence bullets for clarity.

---

## Contributing
Guidance for changelog authors: see `CONTRIBUTING.md`. Author new release entries using the template above and reference related PRs where applicable.
---
title: Changelog
description: Herald project release notes and version history
---
...
### Added
- Placeholder for unreleased features (template below)
...
- External trade adoption: `position/trade_manager.py` now detects, adopts, and manages trades not placed by Herald (configurable adoption policy).
- Command-line trade management: `scripts/trade_cli.py` enables placing, listing, and closing positions from the CLI.
- Predefined trading mindsets: `config/mindsets.py` adds conservative/balanced/aggressive risk profiles.
- Pydantic-based configuration validation: `config_schema.py` centralizes typed config models and environment variable overrides.
- Observability endpoints: `observability/health.py` and `observability/prometheus.py` provide health checks and Prometheus metrics.
### Added
- External trade adoption: `position/trade_manager.py` now detects, adopts, and manages trades not placed by Herald (configurable adoption policy).
- Command-line trade management: `scripts/trade_cli.py` enables placing, listing, and closing positions from the CLI (including dry-run mode for simulation).
  Example:
  ```bash
  herald-trade --symbol EURUSD --side SELL --volume 0.1 --sl 1.0850 --tp 1.0750
  herald-trade --list
  ...
  herald-trade --symbol BTCUSD# --side BUY --volume 0.01 --dry-run
  ```
- Predefined trading mindsets: `config/mindsets.py` provides conservative/balanced/aggressive risk profiles for easier configuration.
- Pydantic-based configuration validation: `config_schema.py` centralizes typed configuration models and supports environment variable overrides.
- Observability endpoints: `observability/health.py` and `observability/prometheus.py` offer health checks and metrics for production observability.
- External trade adoption: `position/trade_manager.py` now detects, adopts, and manages trades not placed by Herald (configurable adoption policy).
- Command-line trade management: `scripts/trade_cli.py` enables placing, listing, and closing positions from the CLI.
- Predefined trading mindsets: `config/mindsets.py` adds conservative/balanced/aggressive risk profiles.
- Pydantic-based configuration validation: `config_schema.py` centralizes typed config models and environment variable overrides.
...
- Position reconciliation at startup: Herald now reconciles pre-existing positions and applies exit strategies consistently.
- Exit strategy implementation: strategies now consistently return `ExitDecision` and prioritized semantics are standardized.
### Changed
- Position reconciliation at startup now detects and tracks positions from prior sessions and applies exit strategies consistently.
- Exit strategy implementation normalized: strategies return `ExitDecision` consistently and use a standardized priority model.
- Position reconciliation at startup: Herald now reconciles pre-existing positions and applies exit strategies consistently.
- Exit strategy implementation: strategies now consistently return `ExitDecision` and prioritized semantics are standardized.

- Pre-commit hooks and `detect-secrets` integration. Sensitive fields are masked in logs and a secrets baseline is introduced.
### Security
- Pre-commit hooks and `detect-secrets` integration. Sensitive fields are masked in logs with a `.secrets.baseline` baseline added for auditing.
- Pre-commit hooks and `detect-secrets` integration. Sensitive fields are masked in logs and a secrets baseline is introduced.
...
- Exit strategy return-type and priority ordering issues.
- Default pip value in `get_pnl_pips()` replaced with a sensible default and reconciled in callers.
- README configuration examples corrected to use JSON.
### Fixed
- Exit strategy return-type and priority ordering issues resolved.
- Default pip value in `get_pnl_pips()` now has a sane default and callers were updated accordingly.
- README examples updated to reflect JSON configuration examples where applicable.
- Exit strategy return-type and priority ordering issues.
- Default pip value in `get_pnl_pips()` replaced with a sensible default and reconciled in callers.
- README configuration examples corrected to use JSON.

- Test suite additions and updates (unit tests for trade manager, exit strategies, etc.), all previous tests remain passing.
### Testing
- The project's test suite was extended and updated (unit tests for trade manager, exit strategies, and indicators). All previous tests remained passing during these updates.
- Test suite additions and updates (unit tests for trade manager, exit strategies, etc.), all previous tests remain passing.
...
### Summary
Phase 2 completion: Production-ready autonomous trading execution with verified MT5 integration, robust testing, and infrastructure for live deployments.

- New indicators, e.g. ATR, and enhancements to existing indicators.
- Exit manager architecture and robust exit-strategy primitives (stop-loss, take-profit, trailing stop, time-based exit).
- CI/CD: GitHub Actions integration, multi-version Python testing, linting and coverage.
### Added
- New indicators including ATR and enhancements to existing indicators.
- Exit manager architecture and robust exit-strategy primitives (stop-loss, take-profit, trailing stop, time-based exit).
- CI/CD: GitHub Actions integration with multi-version Python testing, linting checks and coverage reporting.
- New indicators, e.g. ATR, and enhancements to existing indicators.
- Exit manager architecture and robust exit-strategy primitives (stop-loss, take-profit, trailing stop, time-based exit).
- CI/CD: GitHub Actions integration, multi-version Python testing, linting and coverage.

- Test suite architecture: integration tests are visible and use proper pytest fixtures; refactor of test function patterns.
- Documentation updates and updated version badges and status annotations.
### Changed
- Test suite architecture revised: integration tests are now visible and managed with proper pytest fixtures.
- Documentation updates and version badge/status refinements.
- Test suite architecture: integration tests are visible and use proper pytest fixtures; refactor of test function patterns.
- Documentation updates and updated version badges and status annotations.

- Pytest conventions and MT5 connection flakes; improved resilience in data retrieval and connection lifecycle management.
### Fixed
- Pytest conventions refactored and MT5 connection resilience improved.
- Pytest conventions and MT5 connection flakes; improved resilience in data retrieval and connection lifecycle management.

- Credentials externalized to environment variables and import-time resilience for development environments without MT5.
### Security & Stability
- MT5 credentials moved to environment variables and import-time resilience added for development environments without MT5.
- Credentials externalized to environment variables and import-time resilience for development environments without MT5.
...
### Added
- Feature: one-sentence summary, file path(s) if relevant

### Changed
- Notable behavior or configuration changes

### Fixed
- Important bugfixes with context

### Security
- Security specific changes or advisories

---

## How we structure entries
- Follow Keep a Changelog (https://keepachangelog.com).
- Add concise entries grouped by Added/Changed/Fixed/Security for clarity.
- Use the template above for new releases.

## License
This changelog is part of the Herald repository and inherits the repo license.
# Changelog
