...
Highlights
- Trade adoption and management: Cthulhu can now detect and take control of trades entered outside of the bot.
- Manual trade CLI: `cthulhu-trade` provides simple CLI tooling to place, list, and close trades.
- Pydantic-based configuration validation and predefined trading mindsets for quick risk profiles.
- Startup reconciliation: Cthulhu now automatically detects and tracks positions from previous sessions.
...
Notes for Users
- If you run Cthulhu in an environment where you place manual trades, consider enabling `orphan_trades` (now called adoption) in your config and tune `adopt_symbols` / `ignore_symbols`, `max_age_hours`, and `log_only` for safe adoption.
- `cthulhu-trade` will use Cthulhu's magic number for placed trades, so they will be tracked by the bot.
...
CLI Usage Examples
- `cthulhu-trade list`
- `cthulhu-trade buy EURUSD 0.01 --dry-run`
- `cthulhu-trade close EURUSD --all`
- Health check: `curl http://localhost:8080/health`

Production Notes
- Use `log_only: true` initially for adoption testing
- Ensure `cthulhu-trade` uses the same `config.json`
