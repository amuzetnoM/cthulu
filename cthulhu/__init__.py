"""Compatibility shim package named ``cthulhu``.

This package points its import path to the project root so that imports
like ``cthulhu.connector`` resolve to the modules in the repo (which live
in the directory named ``cthulu``). This keeps the original filesystem
layout while satisfying imports that use the double-L name.
"""
import os

# Make this package's search path include the project root (parent dir).
# That allows `import cthulhu.connector` to find `connector/` under the
