...
class AdvisoryManager:
    ...
    def decide(self, order_req: OrderRequest, signal: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if self.mode == 'ghost':
            ...
            # Proceed to place a tiny test order
            try:
                ...
                logger.info('Ghost mode: placing test order %s %s %s', ghost_req.side, ghost_req.volume, ghost_req.symbol)
                res = None
                try:
                    if not self.log_only:
                        res = self.execution_engine.place_order(ghost_req)
                        self.ghost_trades_count += 1
                except Exception as e:
                    # Log but do not cause a hard reject; record and return as ghost attempt with error
                    payload['decision'] = 'ghost'
                    payload['error'] = str(e)
                    self._record_ml('advisory.signal', payload)
                    logger.exception('Ghost order attempt failed')
                    return {'action': 'ghost', 'result': None}

                payload['decision'] = 'ghost'
                status = getattr(res, 'status', None) if res else None
                status_name = status.name if hasattr(status, 'name') else (str(status) if status is not None else None)
                payload['result'] = {'status': status_name, 'order_id': getattr(res, 'order_id', None) if res else None}
                self._record_ml('advisory.signal', payload)
                return {'action': 'ghost', 'result': res}
            except Exception as e:
                payload['decision'] = 'reject'
                payload['reason'] = str(e)
                self._record_ml('advisory.signal', payload)
                logger.exception('Ghost order failed')
                return {'action': 'reject', 'result': None}
