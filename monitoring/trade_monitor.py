"""
Background trade monitor adapted for Cthulu:
- Polls tracked positions
- Verifies SL/TP readbacks and schedules/reporting retries
- Records ML events for position snapshots
- Exposes a simple API to start/stop the monitor
"""
...
class TradeMonitor:
    ...
    def _scan_and_record(self):
        ...
        for ticket, pos in list(self.position_manager._positions.items()):
            try:
                # Read back values from PositionInfo
                sl = getattr(pos, 'stop_loss', None)
                tp = getattr(pos, 'take_profit', None)
                unrealized = getattr(pos, 'unrealized_pnl', None)
            ...
            if self.news_manager:
                ...
                for e in events:
                    try:
                        ...
                        if importance == 'high' or (getattr(e, 'provider', None) and 'tradingeconomics' in getattr(e, 'provider', '').lower() and importance is None):
                            logger.info(f'High-impact event detected: {e.headline}')
                            # Record ML alert
                            if self.ml_collector:
                                self.ml_collector.record_event('monitor.news_alert', {'provider': getattr(e, 'provider', None), 'headline': getattr(e, 'headline', None), 'symbol': getattr(e, 'symbol', None), 'ts': getattr(e, 'ts', None), 'importance': importance})
                            # Pause trading if trade_manager available
                            if self.trade_manager:
                                try:
                                    self.trade_manager.pause_trading(self.news_alert_window)
                                except Exception:
                                    logger.exception('Failed to pause trading on news alert')
                    except Exception:
