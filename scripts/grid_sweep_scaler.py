...
def main():
    min_time_options = [0, 1, 2, 4]
    trail_options = [0.3, 0.5, 0.7]
    concurrency_options = [1, 3, 5]
    ...
    for min_time, trail_pct, concurrency in combos:
        ...
        metrics = run_simulation(cfg, backtest_max_positions=concurrency)
        result = {
        ...
        log.info(f"Combo min_time={min_time} trail={trail_pct} concurrency={concurrency} -> closed_vol={metrics['closed_volume_total']:.4f} profit_est={metrics['total_profit_estimate']:.2f} first_close={metrics['first_close_bar']}")
    ...
    (out / 'grid_sweep_scaler.json').write_text(json.dumps(ranked, indent=2, default=str))

    print('Top 5 configurations:')
    for r in ranked[:5]:
        m = r['metrics']
        print(f"min_time={r['min_time_in_trade_bars']} trail={r['trail_pct']} conc={r['concurrency']} -> profit_est={m['total_profit_estimate']:.2f} closed_vol={m['closed_volume_total']:.4f} final_vol={m['final_remaining_volume']:.4f} first_close={m['first_close_bar']}")
